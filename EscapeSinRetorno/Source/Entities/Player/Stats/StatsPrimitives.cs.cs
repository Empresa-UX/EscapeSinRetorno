...
namespace EscapeSinRetorno.Source.Systems.Stats
{
    ...
    public readonly struct DamageRequest
    {
        ...
        { Amount = MathF.Max(0, amount); Type = type; IgnoresArmor = ignoresArmor; }
    }
    ...
    public sealed class Stat
    {
        ...
        { Max = MathF.Max(1f, max); Current = MathHelper.Clamp(current, Min, Max); RegenPerSec = regenPerSec; }
        ...
        public void SetMax(float max, bool keepRatio = true)
        {
            ...
            else { Max = max; Current = MathHelper.Clamp(Current, Min, Max); }
        }
        ...
        { if (Math.Abs(RegenPerSec) > 0.0001f) Add(RegenPerSec * dt); }
    ...
    public sealed class BleedingEffect : IStatusEffect
    {
        ...
        { _duration = duration; _tickInterval = MathF.Max(0.1f, tickInterval); _damagePerTick = MathF.Max(0f, dmgPerTick); }

        public void OnApply(PlayerStats stats) { }
        public void Update(PlayerStats stats, float dt)
        {
            _duration -= dt; _tickTimer += dt;
            if (_tickTimer >= _tickInterval)
            { _tickTimer -= _tickInterval; stats.ApplyDamage(new DamageRequest(_damagePerTick, DamageType.Bleed, true)); }
        }
    ...
    public sealed class PoisonEffect : IStatusEffect
    {
        ...
        { _duration = duration; _dps = MathF.Max(0f, dps); }
        ...
        public void Update(PlayerStats stats, float dt)
        {
            ...
            stats.HealthRegenBlocked = true; // why: veneno bloquea curación natural
        }
        public void OnRemove(PlayerStats stats) { stats.HealthRegenBlocked = false; }
    }
    ...
    public sealed class RegenerationEffect : IStatusEffect
    {
        ...
        { _duration = duration; _bonusRegen = bonusRegen; }

        public void OnApply(PlayerStats stats) { stats.TempHealthRegenBonus += _bonusRegen; }
        public void Update(PlayerStats stats, float dt) { _timer += dt; }
        public void OnRemove(PlayerStats stats) { stats.TempHealthRegenBonus -= _bonusRegen; }
    }
    ...
    public sealed class PlayerStats
    {
        ...
        public void Tick(float dt, bool isSprinting, out VisualEffectState fx)
        {
            ...
            if (IsDead)
            {
                ...
                {
                    IsDead = true,
                    VignetteIntensity = 1f,        // oscurecer fuerte si quieres
            ...
            if (isSprinting && Stamina.Current > 0.5f) Stamina.Sub(25f * dt);
            else Stamina.Update(dt);
            ...
            if (Hunger.IsZero) ApplyDamage(new DamageRequest(_cfg.HungerZeroHealthDps * dt, DamageType.True, true));
            if (Thirst.IsZero) ApplyDamage(new DamageRequest(_cfg.ThirstZeroHealthDps * dt, DamageType.True, true));

            // Efectos
            if (_effects.Count > 0)
            {
                var list = new List<IStatusEffect>(_effects.Values);
                foreach (var e in list) { e.Update(this, dt); if (e.IsFinished) RemoveEffect(e.Id); }
            }

            if (!IsDead && Health.IsZero)
            { IsDead = true; OnDeath?.Invoke(); }

            if (Math.Abs(prevHealth - Health.Current) > 0.0001f) OnHealthChanged?.Invoke(Health.Current);

            fx = new VisualEffectState
            {
                IsDead = IsDead,
                VignetteIntensity = Health.IsLow(_cfg.LowHealthPct) ? MathHelper.Lerp(0.2f, 1f, 1f - Health.Ratio / _cfg.LowHealthPct) : 0f,
                DistortionIntensity = Sanity.IsLow(_cfg.LowSanityPct) ? MathHelper.Lerp(0.1f, 0.85f, 1f - Sanity.Ratio / _cfg.LowSanityPct) : 0f,
        ...
        {
            if (IsDead || req.Amount <= 0f) return;
        ...
        { if (IsDead || amount <= 0f) return; Health.Add(amount); }

        public void AddEffect(IStatusEffect effect)
        {
            if (_effects.TryGetValue(effect.Id, out var existing))
            { existing.OnRemove(this); _effects.Remove(effect.Id); }
        ...
        public void RemoveEffect(string id)
        {
            ...
            { e.OnRemove(this); _effects.Remove(id); OnEffectRemoved?.Invoke(id); }
        }

        public void ConsumeFood(float calories = 25f, float sanityBoost = 2f)
        { Hunger.Add(calories); Sanity.Add(sanityBoost); }
        public void Drink(float water = 30f)
        { Thirst.Add(water); }
        public void UseBandage()
        { RemoveEffect("bleeding"); Heal(10f); }
        public void UseAntidote()
        { RemoveEffect("poison"); }
        public void UseTonic()
        { AddEffect(new RegenerationEffect()); }
        public void SetArmorPercent(float pct) => ArmorPercent = MathHelper.Clamp(pct, 0f, 0.9f);
    }
}